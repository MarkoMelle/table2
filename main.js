...
var Table = /*#__PURE__*/function () {
  ...
  _createClass(Table, [{
    ...
    value: function init(json) {
      ...
      var reDrawThead = function reDrawThead() {
        for (var i = 0; i < _this.theadRow.children.length; i += 1) {
          _this.theadRow.children[i].textContent = _this.theadRow.children[i].textContent.replace('⬇️', '');
        }
      };
      this.tbody = this.container.querySelector('tbody');
      this.theadRow = this.container.querySelector('thead tr');
      this.data = JSON.parse(json);
      (_this$tbody = this.tbody).append.apply(_this$tbody, _toConsumableArray(this.getSort()));
      this.container.querySelector('.id').addEventListener('click', function () {
        var _this$tbody2;
        _this.tbody.replaceChildren();
        reDrawThead();
        (_this$tbody2 = _this.tbody).append.apply(_this$tbody2, _toConsumableArray(_this.getSort('id')));
        _this.container.querySelector('.id').textContent += '⬇️';
      });
      this.container.querySelector('.title').addEventListener('click', function () {
        var _this$tbody3;
        _this.tbody.replaceChildren();
        reDrawThead();
        (_this$tbody3 = _this.tbody).append.apply(_this$tbody3, _toConsumableArray(_this.getSort('title')));
        _this.container.querySelector('.title').textContent += '⬇️';
      });
      this.container.querySelector('.year').addEventListener('click', function () {
        var _this$tbody4;
        _this.tbody.replaceChildren();
        reDrawThead();
        (_this$tbody4 = _this.tbody).append.apply(_this$tbody4, _toConsumableArray(_this.getSort('year')));
        _this.container.querySelector('.year').textContent += '⬇️';
      });
      this.container.querySelector('.imdb').addEventListener('click', function () {
        var _this$tbody5;
        _this.tbody.replaceChildren();
        reDrawThead();
        (_this$tbody5 = _this.tbody).append.apply(_this$tbody5, _toConsumableArray(_this.getSort('imdb')));
        _this.container.querySelector('.imdb').textContent += '⬇️';
      });
      this.container.querySelector('.id').click();
    }
  }, {
    key: "getSort",
    value: function getSort(filter) {
      var trArr = [];
      var filterHandler = function filterHandler(Arr) {
        if (filter === 'id') {
        ...
        if (filter === 'title') {
          return Arr.sort(function (a, b) {
            return a.title.slice(0, 1).charCodeAt() - b.title.slice(0, 1).charCodeAt();
          });
        }
        return Arr;
      };
      ...
      this.data.forEach(function (i) {
        ...
        year.textContent = "(".concat(i.year, ")");
        imdb.textContent = "imdb:".concat(i.imdb.toFixed(2));
...
;// CONCATENATED MODULE: ./src/js/json.js
